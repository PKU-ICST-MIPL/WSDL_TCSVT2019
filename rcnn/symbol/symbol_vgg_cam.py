...
def get_vgg_test(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    ## group output
    #group = mx.symbol.Group([rois, cls_prob, bbox_pred, fixed_cls_score, fixed_bbox_pred])
...
def get_vgg_train(num_classes=config.NUM_CLASSES, num_anchors=config.NUM_ANCHORS):
    ...
    #group = mx.symbol.Group([rpn_cls_prob, rpn_bbox_loss, fixed_cls_prob, fixed_bbox_loss , mx.symbol.BlockGrad(label), cls_prob, bbox_loss])
