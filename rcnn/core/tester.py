...
def im_detect(predictor, data_batch, data_names, scale):
    ...
    et = time.time()
    print 'rois{:.4f}s'.format(et - st)
    # save output
    scores = output['cls_prob_reshape_output'].asnumpy()[0]
    fixed_scores = output['fixed_cls_score_output'].asnumpy()[0]
    et1 = time.time()
    print 'scores{:.4f}s'.format(et1 - st)
    bbox_deltas = output['bbox_pred_reshape_output'].asnumpy()[0]
    #et = time.time()
    #print 'scores{:.4f}s'.format(et - st)
    
    fixed_bbox_deltas = output['fixed_bbox_pred_reshape_output'].asnumpy()[0]

    # post processing
    pred_boxes = bbox_pred(rois, bbox_deltas)
    pred_boxes = clip_boxes(pred_boxes, im_shape[-2:])
    fixed_pred_boxes = bbox_pred(rois, fixed_bbox_deltas)
    fixed_pred_boxes = clip_boxes(fixed_pred_boxes, im_shape[-2:])
    fixed_pred_boxes = fixed_pred_boxes / scale
    # we used scaled image & roi to train, so it is necessary to transform them back
    pred_boxes = pred_boxes / scale
    et2 = time.time()
    print 'im_detect{:.4f}s'.format(et2 - st)
    #print(scores)
    return scores, fixed_pred_boxes, data_dict, (et1-st)
    #return scores, pred_boxes, data_dict, (et1-st)
...
def pred_eval(predictor, test_data, imdb, vis=False, thresh=1e-3):
    ...
    for im_info, data_batch in test_data:
        ...
        scale = im_info[0, 2]
        scores, boxes, data_dict, time_part = im_detect(predictor, data_batch, data_names, scale)
        time_total = time_total + time_part
        t2 = time.time() - t
        t = time.time()
        ...
        for j in range(1, imdb.num_classes):
            ...
            cls_scores_idx = np.argsort(-cls_scores, axis=0)
            if len(cls_scores_idx) > 0:
                cls_dets = np.hstack((cls_boxes[cls_scores_idx[0],:], cls_scores[cls_scores_idx[0],:]))
            else:
                cls_dets = np.hstack((cls_boxes, cls_scores))
        ...
        t = time.time()
        totalt = totalt + t2 + t3
        print 'testing {}/{} data {:.4f}s net {:.4f}s post {:.4f}s ave {:.4f}s'.format(i, imdb.num_images, t1, t2, t3, time_total/(i+1))
        i += 1
    print 'total time: {:.4f}s'.format(totalt)
